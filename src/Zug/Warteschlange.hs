...
-}
module Zug.Warteschlange (Warteschlange(), leer, einzelelement, vonListe, anhängen, zeigeErstes, zeigeLetztes, Anzeige(..)) where
...
data Anzeige a = Leer | Gefüllt a (Warteschlange a)
...
zeigeErstes (Warteschlange {eingabe, ausgabe=[]})    = let (a:ausgabe) = reverse eingabe in Gefüllt a (Warteschlange {eingabe=[], ausgabe})
zeigeErstes (Warteschlange {eingabe, ausgabe=(h:t)}) = Gefüllt h (Warteschlange {eingabe, ausgabe=t})

-- | Erhalte das zuletzt hinzugefügte Element und die 'Warteschlange' vor hinzufügen des selben. Effizenz ist im schlimmsten Fall O(n).
...
zeigeLetztes (Warteschlange {eingabe=(h:t), ausgabe}) = Gefüllt h (Warteschlange {eingabe=t, ausgabe})
...
instance Functor Warteschlange where
    ...
    fmap f (Warteschlange {eingabe=e1, ausgabe=a1}) = Warteschlange {eingabe=f <$> e1, ausgabe=f <$> a1}
