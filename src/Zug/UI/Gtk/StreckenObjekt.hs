...
import Zug.UI.Base (
    ...
    bahngeschwindigkeiten, streckenabschnitte, weichen, kupplungen, sprache,
    ausführenMöglich, entfernenBahngeschwindigkeit,
...
entferneHinzufügenWegstreckeWidgets :: forall s r m.
    (WegstreckenElement s, DynamischeWidgetsReader r m, MonadIO m) => s -> m ()
...
streckenabschnittPackNew :: forall m. (ObjektReader ObjektGui m, MonadIO m) =>
    Streckenabschnitt -> MStatusGuiT m STWidgets
...
instance WegstreckenElement (ZugtypEither WEWidgets) where
    ...
    boxWegstrecke ::
        ZugtypEither Weiche -> Lens.Getter DynamischeWidgets (BoxWegstreckeHinzufügen (ZugtypEither WEWidgets))
...
weichePackNew :: forall m z.
    ...
    WegstreckenElement (WEWidgets z) , PlanElement (WEWidgets z)) =>
        Weiche z -> MStatusGuiT m (WEWidgets z)
weichePackNew weiche = do
    ...
    pure weWidgets
        where
            richtungsButtonsPackNew :: forall b m z. (MitBox b, MonadIO m, ObjektReader ObjektGui m) =>
                Weiche z -> b -> TVar (Maybe [Sprache -> IO ()]) -> m ()
            ...
            richtungsButtonsPackNew
                ...
                tvar
                    = void $ do
                        ...
                        boxPackWidgetNewDefault box $ buttonNewWithEventLabel justTVar (anzeige richtung1) $
                            flip runReaderT objektReader $
                                ausführenStatusVarAktion (Stellen weiche richtung1) statusVar
                        boxPackWidgetNewDefault box $ buttonNewWithEventLabel justTVar (anzeige richtung2) $
                            flip runReaderT objektReader $
...
kupplungPackNew :: forall m. (ObjektReader ObjektGui m, MonadIO m) =>
    Kupplung -> MStatusGuiT m KUWidgets
...
wegstreckePackNew :: forall m z. (ObjektReader ObjektGui m, MonadIO m, ZugtypKlasse z, PlanElement (WSWidgets z)) =>
    Wegstrecke z -> MStatusGuiT m (WSWidgets z)
wegstreckePackNew
    ...
    liftIO $ Gtk.on buttonAusführen Gtk.buttonActivated $ flip runReaderT objektReader $
        auswertenStatusVarIOStatus (ausführenMöglich plan) statusVar >>= \case
            ...
            (AnschlüsseBelegt anschlüsse)   -> void $ do
                liftIO $ flip leseSprache spracheGui $ \sprache -> Gtk.set dialogGesperrt 
