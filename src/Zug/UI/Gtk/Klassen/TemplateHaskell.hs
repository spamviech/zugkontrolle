...
erzeugeKlasse abhängigkeiten name = do
  ...
  where
    ...
    istTyp name = TH.lookupTypeName name >>= \case
        Nothing -> pure False
        (Just typName) -> (\case {(TH.TyConI _dec) -> True; _ -> False}) <$> TH.reify typName
    ...
    getterDeklaration =
        TH.ValD (TH.VarP getterName) (TH.NormalB $ TH.AppE (TH.VarE $ TH.mkName "Lens.to") (TH.VarE funktionName)) []