...
i2cWrite :: I2CAddress -> BitValue -> I2CMapIO ()
i2cWrite i2cAddress bitValue tvarI2CKanäle = do
...
i2cWriteAdjust :: I2CAddress -> (BitValue -> BitValue) -> I2CMapIO ()
i2cWriteAdjust i2cAddress bitValueFunktion tvarI2CKanäle = do
    (_fileHandle, oldBitValue) <- i2cKanalLookup i2cAddress tvarI2CKanäle
    i2cWrite i2cAddress (bitValueFunktion oldBitValue) tvarI2CKanäle

-- | Lese den aktuellen Wert aus einem I2C-Kanal
i2cRead :: I2CAddress -> I2CMapIO BitValue
i2cRead i2cAddress tvarI2CKanäle = do
    (fileHandle, _setBitValue) <- i2cKanalLookup i2cAddress tvarI2CKanäle
