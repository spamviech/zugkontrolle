...
import Zug.Plan (Plan, PlanAllgemein(..))
...
import Zug.UI.Cmd.Parser.Anfrage
      ...
      , StatusAnfrageObjekt(..), statusAnfrageObjekt, zuObjekt, StatusAnfrageObjektZugtyp(..)
      , statusAnfrageObjektZugtyp, ObjektZugtyp(..), unbekanntShowText
      , zeigeAnfrageFehlgeschlagenStandard, wähleBefehl, wähleErgebnis, AnfrageFortsetzung(..)
      , verwendeAnfrageFortsetzung, ($<<))
import Zug.UI.Cmd.Parser.Plan
       (AnfragePlan(..), AnfrageAktion(..), AnfrageAktionBahngeschwindigkeit(..)
      , AnfrageAktionStreckenabschnitt(..), AnfrageAktionWeiche(..), AnfrageAktionKupplung(..)
      , AnfrageAktionWegstrecke(..))
...
instance Anzeige AnfrageBefehl where
    ...
    anzeige (ABStatusAnfrageMärklin anfrageKonstruktor _eitherF) =
        anzeige $ anfrageKonstruktor leeresToken
    anzeige (ABStatusAnfrageLego anfrageKonstruktor _eitherF) =
        anzeige $ anfrageKonstruktor leeresToken
...
instance Anfrage AnfrageBefehl where
    ...
    zeigeAnfrage (ABStatusAnfrage anfrageKonstruktor _eitherF) =
        zeigeAnfrage $ anfrageKonstruktor leeresToken
    zeigeAnfrage (ABStatusAnfrageMärklin anfrageKonstruktor _eitherF) =
        zeigeAnfrage $ anfrageKonstruktor leeresToken
    zeigeAnfrage (ABStatusAnfrageLego anfrageKonstruktor _eitherF) =
        zeigeAnfrage $ anfrageKonstruktor leeresToken
    ...
    zeigeAnfrageOptionen ABSprache = Just $ toBefehlsString . \sprache
        -> map (`anzeige` sprache) Language.alleSprachen
    zeigeAnfrageOptionen (ABAktionPlan _plan) = Just $ toBefehlsString . Language.aktionPlan
    zeigeAnfrageOptionen (ABAktionPlanAusführend _plan _neu) =
        Just $ toBefehlsString . Language.aktionPlanAusführend
    zeigeAnfrageOptionen (ABAktionPlanGesperrt _plan _neu _pins) =
...
instance MitAnfrage (Either BefehlSofort Befehl) where
    ...
    anfrageAktualisieren :: AnfrageBefehl
                         -> EingabeToken
                         -> AnfrageFortsetzung AnfrageBefehl (Either BefehlSofort Befehl)
    ...
    anfrageAktualisieren
        ABEntfernen
        token@EingabeToken {eingabe} = case anfrageObjektExistierend token of
    ...
    anfrageAktualisieren ABSpeichern EingabeToken {eingabe} =
        AFErgebnis $ Right $ Speichern $ Text.unpack eingabe
    anfrageAktualisieren ABLaden EingabeToken {eingabe} =
        AFErgebnis $ Left $ BSLaden $ Text.unpack eingabe
    anfrageAktualisieren (ABAktionPlan plan@Plan {plAktionen}) token =
        wähleErgebnis
            token
            [(Lexer.Ausführen, Right $ Ausführen plan zeigeFortschritt $ pure ())]
        where
